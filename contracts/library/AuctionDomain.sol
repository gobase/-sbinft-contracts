...
library AuctionDomain {
  ...
  bytes32 constant ASSET_TYPEHASH =
    keccak256(
      "Asset(bytes4 originKind,address asset,uint256 assetId,uint16 partnerFeeRate,bool isSecondarySale)"
    );
  ...
  bytes32 constant AUCTION_TYPEHASH =
    keccak256(
      "Auction(uint256 auctionId,bytes4 pfKind,address payable creatorAddress,Asset asset,AuctionType auctionType,BidStatus bidStatus,uint256 startPrice,uint256 startTime,uint256 endTime,uint16 pfFeeRate,uint16 externalPfFeeRate,address platformSigner)Asset(bytes4 originKind,address asset,uint256 assetId,uint16 partnerFeeRate,bool isSecondarySale)AuctionType(bytes4 bidMode,bytes4 auctionKind,address paymentToken)BidStatus(address currentTopBidder,uint256 currentPrice,address[] bidderList,uint256[] bidPriceHistory)"
    );
